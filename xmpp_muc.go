...
func (c *Client) SendTopic(chat Chat) (n int, err error) {
	return fmt.Fprintf(c.w, "<message to='%s' type='%s' xml:lang='en'>"+"<subject>%s</subject></message>",
...
func (c *Client) JoinMUCNoHistory(jid, nick string) (n int, err error) {
	...
	return fmt.Fprintf(c.w, "<presence to='%s/%s'>\n"+
		"<x xmlns='%s'>"+
...
func (c *Client) LeaveMUC(jid string) (n int, err error) {
	return fmt.Fprintf(c.w, "<presence from='%s' to='%s' type='unavailable' />",
		c.jid, xmlEscape(jid))
}
